...
logging.basicConfig(level=logging.INFO, filename='../../Downloads/everNoTenkai/ShopBotLogging.log')
logger = logging.getLogger('Logger')


logger.info('App is running')
bot = telebot.TeleBot('6981741347:AAGxJEdy2WKeaPheV3kr5mNUCdcnLVvrTGE')
...
def start(message):
    ...
    if message.chat.id == 64741052136:
        bot.send_message(message.chat.id, 'ку ебанушка', reply_markup=markup_admin())
    else:
    ...
    try:
        if call.message:
            if 'buy' == call.data:
                ...
                bot.delete_message(call.message.chat.id, call.message.message_id)
                bot.send_photo(call.message.chat.id, open('../../Downloads/everNoTenkai/candies.png', 'rb'), reply_markup=markup_candies())

            elif 'go_to_noodles' == call.data:
                bot.delete_message(call.message.chat.id, call.message.message_id)
                bot.send_photo(call.message.chat.id, open('../../Downloads/everNoTenkai/noodles.jpg', 'rb'), reply_markup=markup_noodles())
                ...
                if status == 'ok':
                    ...
                    info = f'{call.message.chat.id} bought {str(basket.my_basket)}'
                    logger.info(info)
                else:
